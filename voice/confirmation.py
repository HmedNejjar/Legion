...
class ConfirmationHandler:
    ...
    def ask_confirmation(self, narration: str, timeout: int = 15) -> bool:
        """Speaks the action narration and listens for a yes/no response."""
        confirmation = f'{narration}, confirm whether i proceed with this action sir'
        self.tts.read(confirmation)
        ...
        if any(word in response_text for word in ['yes', 'yeah', 'ye', 'yep', 'ok', 'sure', 'go', 'do it']): return True
            ...
            return self.ask_confirmation(confirmation, timeout)